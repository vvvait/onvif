...
function matchXAddr(xaddrs, address) {
	var ipMatch = xaddrs.filter(function(xaddr) {
		const ip = xaddr.hostname.split(".");		
		if (ip.length == 4 && ip[2] !== address.split(".")[2]) {
			xaddr.host = xaddr.host.replace(xaddr.hostname, address);
			xaddr.hostname = address;
			xaddr.href = xaddr.href.replace(xaddr.hostname, address);
		}
