...
const { prettifyXml } = require('./utils');
...
var Cam = function(options, callback) {
	...
	this.ignoretimeerror = options.ignoretimeerror || true;
...
Cam.prototype._request = function(options, callback) {
	...
	var req = http.request(reqOptions, function(res) {
		...
		res.on('end', function() {
			...
			parseSOAPString(xml, (err, data, xml) => {
				callback(err, data, { request: prettifyXml(options.body), response: prettifyXml(xml)});
			});		
		});
	});
...
Cam.prototype.getSystemDateAndTime = function(callback) {
	...
	this._request({
		...
		if (err) {
			if (xml && typeof xml === "string" && xml.toLowerCase().includes('sender not authorized')) {
