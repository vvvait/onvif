...
module.exports = function(Cam) {
	...
	*/
	 ...
	 */
	Cam.prototype.getProfiles = function(token, callback) {
		if (typeof token === "function")
			callback = token;
		const body = this._envelopeHeader() +
					'<GetProfiles xmlns="http://www.onvif.org/ver10/media/wsdl"/>' +							
					this._envelopeFooter();		
		this._request({ service: 'media', body }, function(err, data, xml) {
			if (!err) {
				/**
	...
	Cam.prototype.getStreamUri = function(options, callback) {
		...
		this._request({
			...
			if (callback) {
				const response = linerase(data)?.getStreamUriResponse;
				callback.call(this, err, err ? null : response.mediaUri ?? response, xml);
			}
		}.bind(this));
	};
	...
	Cam.prototype.getSnapshotUri = function(options, callback) {
		...
		this._request({
			...
			if (callback) {
				const response = linerase(data)?.getSnapshotUriResponse;
				callback.call(this, err, err ? null : response.mediaUri ?? response, xml);
			}
		}.bind(this));
	};
