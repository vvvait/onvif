...
Cam.prototype.createPullPointSubscription = function(callback) {
	this._request({
		...
		if (!err) {
			...
			this.events.subscription.subscriptionReference.realAddress = this.events.subscription.subscriptionReference.address;
...
Cam.prototype.pullMessages = function(options, callback) {
	...
	this._request({
		...
		'<a:To s:mustUnderstand="1">' + this.events.subscription.subscriptionReference.realAddress + '</a:To>' +
		'<SubscriptionId a:IsReferenceParameter="true" xmlns="http://www.axis.com/2009/event">' +
			this.events.subscription.subscriptionReference.referenceParameters.subscriptionId +
		'</SubscriptionId>' +
		'</s:Header>' +
		...
		'</PullMessages>' +
		/*'<Renew xmlns="http://docs.oasis-open.org/wsn/b-2"><TerminationTime>PT60S</TerminationTime></Renew>' +
		*/this._envelopeFooter()
	}, function(err, res, xml) {
		if (!err) {
